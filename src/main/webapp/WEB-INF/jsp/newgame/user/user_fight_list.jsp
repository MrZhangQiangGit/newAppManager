						  	...
						  	<ul class="am-avg-sm-3">
						  		...
						  		<li>
							    	<div class="">
							    		<span>状态:</span>
							    		<span>
							    			<c:if test="${var.STATUS=='1' }">等待</c:if>
							    			<c:if test="${var.STATUS=='2' }">进行</c:if>
							    			<c:if test="${var.STATUS=='3' }">结束</c:if>
							    		</span>
							    	</div>
						  		</li>
						  		<li>
							    	...
							    	<c:choose>
							    		<c:when test="${var.TYPE=='zhs' }">捉红三</c:when>
							    		<c:otherwise>其他</c:otherwise>
							    	</c:choose>
							    	</i>
							    		<%-- <c:if test="${var.TYPE=='zhs' }"><i class="am-badge am-badge-danger am-round">捉红三</i></c:if> --%>
							    	</div>
						  		</li>
						  	</ul>
						  </div>
...
<script type="text/javascript">
	...
	function loadMore(page,keywords){
		...
		//ajax添加
		$.ajax({
			url:"userFight/loadMoreUFList.do",
			data:{"page":page,"keywords":keywords},
			dataType:"json",
			async:false,//是否异步,默认true为异步,同步请求将锁住浏览器，用户其它操作必须等待请求完成才可以执行。
	...
	function search(){
		...
		var url = "userFight/userFightList.do?keywords="+keywords;
