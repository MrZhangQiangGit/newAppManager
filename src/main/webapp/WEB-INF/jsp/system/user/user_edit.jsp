					...
					<div class="row">
						<div class="col-xs-12">
								<form action="user/${msg}.do" name="userForm" id="userForm" method="post">
									...
									<input type="hidden" name="AGENT_NAME" id = "AGENT_NAME" value="${pd.AGENT_NAME }">
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										...
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;">用户游戏ID:</td>
											<td><input type="text" name="NUMBER" id="NUMBER" value="${pd.NUMBER }" maxlength="32" placeholder="这里输入编号" title="编号"  onblur="hasN('${pd.USERNAME }')" style="width:98%;"  <c:if test="${msg=='editU' }">readonly="readonly"</c:if>/></td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td><input type="password" name="PASSWORD" id="chkpwd"  maxlength="32" placeholder="确认密码" title="确认密码" style="width:98%;"/></td>
										</tr>
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;">姓名:</td>
											<td><input type="text" name="NAME" id="name"  value="${pd.NAME }"  maxlength="32" placeholder="这里输入姓名" title="姓名" style="width:98%;"<c:if test="${pd.loger !=888888}"><c:if test="${msg=='editU'}">readonly="readonly" </c:if></c:if> />
													</td>
										</tr>
										<tr>
										...
										<%-- <tr>
											<td style="width:79px;text-align: right;padding-top: 13px;">所属代理:</td>
											<td>
												<select class="chosen-select form-control" name="AGENT_CODE" id="AGENT_CODE" data-placeholder="这里选择所属代理" style="vertical-align:top;" style="width:98%;">
													<option value=""></option>
													<c:forEach items="${agent}" var="arr">
														<option value="${arr.AGENT_CODE }" <c:if test="${arr.AGENT_CODE == pd.AGENT_CODE }">selected</c:if>>${arr.AGENT_NAME }</option>
													</c:forEach>
												</select>
											</td>
										</tr> --%>
										...
										<input type="hidden" nname="AGENT_CODE" id="AGENT_CODE" value="${arr.AGENT_CODE }" />
										<tr>
...
<script type="text/javascript">
	...
	function save(){
	   ...
	   if($("#chkpwd").val()!=""){
		...
		if($("#user_id").val()==""){
			$("#AGENT_NAME").val($("#AGENT_CODE").find("option:selected").text());
			hasU();
		}else{
			$("#AGENT_NAME").val($("#AGENT_CODE").find("option:selected").text());
			$("#userForm").submit();
