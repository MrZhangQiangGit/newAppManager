...
public class LoginAppController extends BaseController {
	...
	private MnoticeManager noticeService;
...
//		mv.setViewName("newapp/login");
		mv.setViewName("newsystem/login/login");
//		mv.setViewName("system/login.html");
		mv.addObject("pd",pd);
			...
			if(null == code || "".equals(code)){//判断效验码
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){		//判断登录验证码
					...
					if(pd != null){
						...
						session.setAttribute(Const.SESSION_USERNAME, pd.getString("USERNAME"));
...
//				menuList = this.changeMenuF(allmenuList, session, USERNAME, changeMenu);	//切换菜单
				...
				logger.info("用户按钮权限："+Jurisdiction.getHC().toString());
				//查询公告
				PageData pn = noticeService.findOneNotice(pd);
				pd.put("notice", pn.getString("CONTENT"));
				
				
				this.getRemortIP(USERNAME);	//更新登录IP
				mv.setViewName("newsystem/index/index");
//				mv.setViewName("newsystem/index/default");
				mv.addObject("user", user);
//				mv.addObject("menuList", menuList);
			}else {
				//mv.setViewName("html/html/logins/login.html");//session失效后跳转登录页面
				mv.setViewName("newsystem/login/login");//session失效后跳转登录页面
			}
		} catch(Exception e){
//			mv.setViewName("html/html/logins/login.html");
	...
	public ModelAndView logout(){
		...
		pd.put("SYSNAME", Tools.readTxtFile(Const.SYSNAME)); //读取系统名称
//		mv.setViewName("html/html/logins/login.html");
		mv.setViewName("redirect:newapp/login");
		mv.addObject("pd",pd);
		return mv;
	}
