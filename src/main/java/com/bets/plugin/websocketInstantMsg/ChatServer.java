package com.bets.plugin.websocketInstantMsg;
...
//		sendToAll( "new connection: " + handshake.getResourceDescriptor() );
		System.out.println("webSocket连接===" + conn.getRemoteSocketAddress().getAddress().getHostAddress());
	}
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("UserChatNow")){//用户进入开始聊天
			this.userjoin(message.replaceFirst("UserChatNow", ""),conn);
		}if(null != message && message.startsWith("Leave")){//用户离开聊天
			this.userLeave(conn);
